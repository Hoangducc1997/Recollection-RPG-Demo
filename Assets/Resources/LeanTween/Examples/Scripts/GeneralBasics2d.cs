...
public class GeneralBasics2d : MonoBehaviour {
	...
	public UnityEngine.GameObject prefabParticles;

	#if !(UNITY_3_5 || UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)

	void Start () {
        // Setup
        UnityEngine.GameObject avatarRotate = createSpriteDude( "avatarRotate", new Vector3(-2.51208f,10.7119f,-14.37754f));
        UnityEngine.GameObject avatarScale = createSpriteDude( "avatarScale", new Vector3(2.51208f,10.2119f,-14.37754f));
        UnityEngine.GameObject avatarMove = createSpriteDude( "avatarMove", new Vector3(-3.1208f,7.100643f,-14.37754f));
    ...
    UnityEngine.GameObject createSpriteDude( string name, Vector3 pos, bool hasParticles = true ){
		...
		if(hasParticles){
            UnityEngine.GameObject particles = (UnityEngine.GameObject)UnityEngine.GameObject.Instantiate(prefabParticles, Vector3.zero, prefabParticles.transform.rotation );
	...
	void advancedExamples(){
        LeanTween.delayedCall(gameObject, 14f, ()=>{
			for(int i=0; i < 10; i++){
                ...
                // Instantiate Avatar
                UnityEngine.GameObject dude = createSpriteDude( "dude"+ i, new Vector3(-2.51208f,7.100643f,-14.37754f), false);//(GameObject)GameObject.Instantiate(prefabAvatar, Vector3.zero, prefabAvatar.transform.rotation );
